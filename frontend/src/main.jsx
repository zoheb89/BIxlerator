...
function App(){
 ...
 const [dash,setDash]=useState({kpis:{qvd_assets_converted:0,pii_columns_secured:0,dax_expressions:0,avg_conversion_seconds:0},activity:[]})
...
function Hero({nav}){return <section className="hero">
 ...
 <div className="flow"><img src="/qlik-powerbi-logo.svg" alt="Qlik to Power BI migration"/></div>
 </section>}
...
function KPIcards({k}){let data=[['qvd_assets_converted','QVD assets converted','+'+(k.qvd_this_week??0)+' this week','blue'],['pii_columns_secured','PII columns secured','actual protected columns','green'],['dax_expressions','Qlik expressions → DAX','+'+(k.dax_this_week??0)+' this week','purple'],['avg_conversion_seconds','Avg. conversion time','completed assets','cyan']];return data.map(([key,label,delta,c])=><div className="kpi-card" key={key}><div className={'kicon '+c}>{icons[key==='qvd_assets_converted'?'qvd':key==='pii_columns_secured'?'pii':key==='dax_expressions'?'dax':'activity']}</div><span>{label}</span><strong>{k[key]??0}{key==='avg_conversion_seconds'?'s':''}</strong><small className={c}>{delta}</small><div className="spark"><i/><i/><i/><i/><i/><i/></div></div>)}
